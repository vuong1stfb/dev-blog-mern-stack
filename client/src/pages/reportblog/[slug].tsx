    ...
    if (value === "") {
      ...
      if (value === "Khác") {
            if(textInput === ""){
              ...
              await postAPI("report-blog", {message: textInput, blog_report: slug}, access_token).then(res => {
        ...
        const result = await checkTokenExp(auth.access_token, dispatch);
        const access_token = result ? result : auth.access_token;
        await postAPI("report-blog", {message: value, blog_report: slug}, access_token).then(res => {
          dispatch({ type: ALERT, payload: { loading: false } })
          handleOpen()
        }).catch(err => {
          dispatch({ type: ALERT, payload: { errors: err.response.data.msg } });
              ...
              <Fade in={open}>
                ...
                >
                  <Model>
                    ...
                    <Descmodel>Đơn báo cáo của bạn đã được gửi đi, Vui lòng chờ xử lý...</Descmodel>
                       <button
        ...
        <div className="header">
          ...
          <Content>
            <div className="body">
              <div className="header">
                <div className="reading-list">
                  <h1 style={{ fontWeight: "bold" }}>Báo cáo Blog</h1>
                  <p style={{ fontSize: "20px" }}>
                    Cảm ơn bạn đã báo cáo bất kỳ sự lạm dụng nào vi phạm quy tắc
              ...
              <Containerreport>
                <form style={{width: '100%'}} onSubmit={handleSubmit}>
                  ...
                  >
                    ...
                    >
                      <FormControlLabel
                        value="Blog có nội dung vi phạm cộng đồng"
                        control={<Radio />}
                        label="Blog có nội dung vi phạm cộng đồng"
                      />
                      <FormControlLabel
                      ...
                      />
                      <FormControlLabel
                        value="Kêu gọi bạo loạn"
                        control={<Radio />}
                        label="Kêu gọi bạo loạn"
                      />
                      <FormControlLabel
                        value="Khác"
                        control={<Radio />}
                    ...
                    <button style={{marginTop: '30px'}} className="btn btn-danger" type="submit">Gửi Phản hồi</button>
                  </FormControl>
                </form>
              </Containerreport>
            </div>
