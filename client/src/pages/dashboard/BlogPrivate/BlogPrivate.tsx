...
const BlogPrivate = () => {
  ...
  const publicBlog = async (id: any, index: number) => {
    if(!auth.access_token) return;
    const result = await checkTokenExp(auth.access_token, dispatch);
    const access_token = result ? result : auth.access_token;
    await patchAPI("public-blog", {idblog: id}, access_token).then(async (res) => {
      listblog.splice(index, 1)
      dispatch({ type: ALERT, payload: { success: "Đã đặt bài viết ở chế độ công khai" } })
    }).catch(err => {
  ...
  useEffect(() => {
    async function fectdata() {
      ...
      const result = await checkTokenExp(auth.access_token, dispatch);
      const access_token = result ? result : auth.access_token;
      await getAPI("get-blog-private", auth.access_token).then(res => {
  ...
  return (
    <Container>
      {/* secction@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ */}
      <h2>Bảng điều khiển » Blog riêng tư</h2>

      {/* body@@@@@@@@@@@@@@@@@@@@@@@@@@@@ */}
      <div className="row mt-3">
        <div className="col-md-3">
          <LeftMenu isActive="blog-private"></LeftMenu>
        </div>
                ...
                <div className=" col-12 col-md-11">
                  ...
                  <div style={{ marginTop: "7px" }}>
                              <span>
                                ...
                                : {blog.status === "Private" ? "Chỉ mình tôi" : null}
                              </span>
                            </div>
                </div>
